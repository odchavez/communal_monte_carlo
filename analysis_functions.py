            ...
            for Nt_item in f_Nt:
                for p_item in f_p:
                    for part_num_item in f_part_num:
                        ...
                        #print("completed everything but temp_ao.compute_run_time")
...
def prep_big_results_dict_v2(f_shard_number, f_Xy_N, f_N_Epoch, f_Nt, f_p, f_GP_version, f_part_num, f_predictors, f_exp_num, f_comm):
    wi_comm_list = list()
    no_comm_list = list()
    big_results_dict = {}
    for shard_number_item in f_shard_number:
                ...
                for p_item in f_p:
                    for part_num_item in f_part_num:
                        ...
                        for GP_version_item in f_GP_version:
                            ...
                            path_obj_with_comm_instance = exp_file_path(
                                shard_number_item, 
                                f_Xy_N, 
                                N_Epoch_item, 
                                Nt_item, 
                            ...
                            #print("path_obj_no_comm_instance=",path_obj_no_comm_instance.v2_results_file)
                            
                            both_exist = (
                                os.path.exists(path_obj_with_comm_instance.v2_results_file) 
                                and os.path.exists(path_obj_no_comm_instance.v2_results_file)
                            )
                            ...
                            if both_exist:
                                ...
                                try:
                                    #print("analyze run with comm begin")
                                    w_run = analyze_run_v2(
                                        f_path = path_obj_with_comm_instance.v2_results_file,
                                    ...
                                    #print("analyze run no comm complete")
                                    print("SUCCESS WITH ", path_obj_with_comm_instance.exp_key, " GP_version_item = ", GP_version_item)
                                    print("SUCCESS WITH ", path_obj_no_comm_instance.exp_key, " GP_version_item = ", GP_version_item)
                                    
                                    temp_ao.wi_comm_list.append(w_run)
                                    temp_ao.no_comm_list.append(n_run)
                                    
                                except Exception:
                                    print("****************** Exception: FAILED WITH ONE OR BOTH ******************")
                                    #print(path_obj_with_comm_instance.exp_key, " GP_version_item = ", GP_version_item)
                                    #print(path_obj_no_comm_instance.exp_key, " GP_version_item = ", GP_version_item)
                                    #print("in prep_big_results_dict , w_run.esti_lik=", w_run.esti_lik)
                                    #print(path_obj_instance.exp_key)
                                    #print(path_obj_instance.with_comm_results_file)
                                    #print("in prep_big_results_dict , n_run.esti_lik=", n_run.esti_lik)
                        ...
                        temp_ao.compute_lik_diffs()
                        temp_ao.compute_run_time()
                        big_results_dict[path_obj_with_comm_instance.exp_key] = temp_ao
...
class analyze_run:
    ...
    def get_params_from_results_with_comm(self, path):
        ...
        #print("results_output=", results_output)
        #print("results_output_top.particle_number=",results_output_top.particle_number)
        self.number_of_particles = int(results_output_top.particle_number.iloc[0])
        #print("C1")
        self.number_of_shards = int(results_output_top['N_Node='].iloc[0])
        #print("C2")
        self.number_of_predictors = int(results_output_top['p='].iloc[0])
        #print("D")
    ...
    def time_cleaner(self, path, column_name):
        ...
        if column_name == 'particle_filter_run_time':
            if type(df['particle_filter_run_time'].iloc[0]) == str:
                None
            elif type(df['p='].iloc[0]) == str:
                column_name = 'p='
...
class analyze_run_v2:
    ...
    def get_params_from_results_with_comm(self, path):
        ...
        self.number_of_particles = int(results_output.particle_number[0])
        self.number_of_shards = int(results_output['N_Node='][0])
        self.number_of_predictors = int(results_output['p='][0])
...
class analyze_run_airline:
    
    def __init__(self, f_path, comm, col='final_params'):
        print("in analyze_run __init__")
        self.comm = comm
        print(1)
        #self.true_cols = true_cols
        ...
        print(4)
        #print("True Beta shape = ", self.Beta_true_data.shape)
        self.Beta_com = self.Beta_true_data[self.true_cols][(self.skip_size-1)::self.skip_size]
        self.Beta_com.reset_index(inplace=True)
        #self.predictor_number = self.Betas_in_columns[0].shape[0]
        #self.epoch_number = len(self.Betas_in_columns)
        print(5)
...
class analysis_obj:
    ...
    def compute_run_time(self):
        ...
        for i in range(len(self.wi_comm_list)):
            ...
            self.pf_run_time_array[i] = self.wi_comm_list[i].pf_run_time

            
            self.particle_commm_array[i] = (
...
class exp_file_path:
    def __init__(self, shard_num, Xy_N, N_Epoch_item, Nt_item, p_item, GP_version_item, part_num_item, exp_num=0, comm=False):
        ...
        self.v2_results_file = (
            'experiment_results/synth_data/results_emb_par_fit_test_with_comm'
            '_shard_num=' + str(shard_num) + 
            '_Xy_N='+str(Xy_N) +
            '_Epoch_N=' + str(N_Epoch_item) + 
            '_Nt=' + str(Nt_item) + 
            '_p=' + str(p_item) + 
            '_GP_version=' + str(GP_version_item) + 
            '_part_num=' + str(part_num_item) + 
            '_exp_num=' + str(exp_num) + 
            '_communicate=' + str(comm) + '.csv'
        )
        self.v2_results_no_comm_file = (
            'experiment_results/synth_data/results_emb_par_fit_test_with_comm'
            '_shard_num=' + str(shard_num) + 
            '_Xy_N='+str(Xy_N) +
            '_Epoch_N=6000' + 
        ...
        self.v2_beta_file = (
            ...
            '_p='+str(p_item)+
            '/GP_version='+str(GP_version_item)+
            '/Beta_t_Xy_N='+str(Xy_N)+'_Epoch_N=1200'+#str(N_Epoch_item)+
            '_Nt='+str(Nt_item)+
            '_p='+str(p_item)+
            '_GP_version='+str(GP_version_item)+'.csv'
        )
...
def heat_map_data_prep_pf_run_time_mean(pred_num, part_num, N_Epoch, shard_num, big_results_dict):#, version_count = 10):

    hm_plot_data_pf_run_time = np.zeros((len(N_Epoch), len(part_num)))
        ...
        for pn_index in range(len(part_num)):
            ...
            for k in range(len(dict_keys)):
                ...
                if (cond_1 and cond_2 and cond_3 and cond_4):
                    ...
                    hm_plot_data_pf_run_time[ne_index, pn_index] = (
                        (
                            big_results_dict[dict_keys[k]].mean_pf_run_time
                        )
                    )
                    
    output_mean_pf_run_time = pd.DataFrame(
        hm_plot_data_pf_run_time, 
        index=N_Epoch, 
        columns=part_num
    )
    output_mean_pf_run_time['index']=N_Epoch
    
    return output_mean_pf_run_time


def heat_map_data_prep_pf_run_time_std(pred_num, part_num, N_Epoch, shard_num, big_results_dict):#, version_count = 10):
        ...
        for pn_index in range(len(part_num)):
            ...
            for k in range(len(dict_keys)):
                ...
                if (cond_1 and cond_2 and cond_3 and cond_4):
                    ...
                    hm_plot_data_pf_run_time[ne_index, pn_index] = (
                        (
                            big_results_dict[dict_keys[k]].std_pf_run_time
                        )
                    )
                    
    output_std_pf_run_time = pd.DataFrame(
        hm_plot_data_pf_run_time, 
        index=N_Epoch, 
        columns=part_num
    )
    output_std_pf_run_time['index']=N_Epoch
    
    return output_std_pf_run_time


def heat_map_data_prep_particle_comm_time_mean(pred_num, part_num, N_Epoch, shard_num, big_results_dict):#, version_count = 10):

    hm_plot_data_particle_comm_time = np.zeros((len(N_Epoch), len(part_num)))
        ...
        for pn_index in range(len(part_num)):
            ...
            for k in range(len(dict_keys)):
                ...
                if (cond_1 and cond_2 and cond_3 and cond_4):
                    ...
                    hm_plot_data_particle_comm_time[ne_index, pn_index] = (
                        (
                            big_results_dict[dict_keys[k]].mean_particle_comm_time
                        )
                    )
                    
    output_mean_particle_comm_time = pd.DataFrame(
        hm_plot_data_particle_comm_time, 
        index=N_Epoch, 
        columns=part_num
    )
    output_mean_particle_comm_time['index']=N_Epoch
    
    return output_mean_particle_comm_time


def heat_map_data_prep_particle_comm_time_std(pred_num, part_num, N_Epoch, shard_num, big_results_dict):#, version_count = 10):
        ...
        for pn_index in range(len(part_num)):
            ...
            for k in range(len(dict_keys)):
                ...
                if (cond_1 and cond_2 and cond_3 and cond_4):
                    ...
                    hm_plot_data_particle_comm_time[ne_index, pn_index] = (
                        (
                            big_results_dict[dict_keys[k]].std_particle_comm_time
                        )
                    )
                    
    output_std_particle_comm_time = pd.DataFrame(
        hm_plot_data_particle_comm_time, 
        index=N_Epoch, 
        columns=part_num
    )
    output_std_particle_comm_time['index']=N_Epoch
    
    return output_std_particle_comm_time
